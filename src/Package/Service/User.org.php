...
class User extends Main
{
    ...
    private static function getTokens(App $object, Entity $node): array
    {
        ...
        $expose = \Difference\Fun\Doctrine\Service\Entity::expose_get(
            $object,
            $entity,
            $entity . '.' . $function . '.output'
        );
        $record = [];
        $record = \Difference\Fun\Doctrine\Service\Entity::output(
            $object,
            $node,
            $expose,
            $entity,
    ...
    public static function current(App $object): array
    {
        ...
        if($claims->has('user')){
            ...
            if($uuid && $email){
                ...
                $toArray = \Difference\Fun\Doctrine\Service\Entity::expose_get(
                    $object,
                    $entity,
                    $entity . '.' . $function . '.output'
                );
